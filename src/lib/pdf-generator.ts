...
async function generateHighQualityGrayscalePdf(
  pages: Page[],
  ...
  for (let i = 0; i < numSheets; i++) {
    ...
    for (let j = startIndex; j < endIndex; j++) {
      ...
      for (let k = 0; k < data.length; k += 4) {
        ...
        const baseLuma = isInverted ? 255 - luma : luma;
...
export async function generatePdf(
  ...
  // --- HIGH-QUALITY INVERT / B&W / GRAYSCALE PIPELINE ---
  if (customization.colorMode.invert || customization.colorMode.bw || customization.colorMode.grayscale) {
      return generateHighQualityGrayscalePdf(pages, customization, setProgress, customization.colorMode.invert);
  }
  
  // --- EXISTING/NORMAL (COLOR) PIPELINE ---
  else {
    let pagesToProcess = pages.filter((p) => p.selected);
    setProgress(5);
