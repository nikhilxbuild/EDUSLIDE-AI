          ...
          <div className="space-y-8 md:col-span-2">
              ...
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                <div className="space-y-3">
                  <Label htmlFor="rows-slider" className="flex items-center">
                    <Rows className="mr-2" /> Rows: {customization.rows}
                  </Label>
                  <Slider
                    id="rows-slider"
                    value={[customization.rows]}
                    onValueChange={([val]) =>
                      setCustomization({ ...customization, rows: val })
                    }
                    min={1}
                    max={8}
                    step={1}
                ...
                <div className="space-y-3">
                  <Label htmlFor="cols-slider" className="flex items-center">
                    <Columns className="mr-2" /> Columns: {customization.cols}
                  </Label>
                  <Slider
                    id="cols-slider"
                    value={[customization.cols]}
                    onValueChange={([val]) =>
                      setCustomization({ ...customization, cols: val })
                    }
                    min={1}
                    max={8}
                    step={1}
            ...
            <div className="grid grid-cols-1 gap-8 sm:grid-cols-2">
              <div className="space-y-4">
               ...
               {/* Color Mode */}
              <div className="space-y-4">
                <h3 className="font-semibold flex items-center"><Palette className="mr-2" />Color Mode</h3>
                <RadioGroup
            ...
            <Separator />
             {/* Auto Clean-up */}
            <div className="space-y-4">
               <h3 className="font-semibold flex items-center"><Wand2 className="mr-2" />Auto Clean-Up</h3>
               <div className="flex items-center space-x-4">
                <div className="flex items-center space-x-2">
                  <Switch id="remove-blank" checked={customization.removeBlankPages} onCheckedChange={(checked) => setCustomization({...customization, removeBlankPages: checked})}/>
                  <Label htmlFor="remove-blank">Remove blank pages</Label>
                </div>
                <div className="flex items-center space-x-2">
                   <Switch id="crop-borders" checked={customization.cropBorders} onCheckedChange={(checked) => setCustomization({...customization, cropBorders: checked})} />
